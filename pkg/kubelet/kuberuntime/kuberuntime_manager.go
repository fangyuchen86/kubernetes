...
func (m *kubeGenericRuntimeManager) computePodActions(pod *v1.Pod, podStatus *kubecontainer.PodStatus) podActions {
	...
	for idx, container := range pod.Spec.Containers {
		...
		if _, _, changed := containerChanged(&container, containerStatus); changed {
			...
			container.CustomProbes, container.CustomProbeStatus); found && int(customLiveness.RestartCount) == containerStatus.RestartCount && customLiveness.Status == v1.CustomProbeFailure {
			...
			container.CustomProbes, container.CustomProbeStatus); found && int(customStartup.RestartCount) == containerStatus.RestartCount && customStartup.Status == v1.CustomProbeFailure {
